...
fn process_single_project(proj_path: &std::path::Path) -> Result<(), String> {
    ...
    // === 后续阶段（导出、渲染监控、拉取）待实机调试后完善 ===

    // 阶段 6: 打开最新导入的项目并执行导出
    logger::step("[6/7] 打开项目并执行导出（待完善）");
    // TODO: 在项目列表中找到刚导入的项目，点击打开
    //       然后找到导出按钮，配置导出参数，开始渲染
    logger::warn("导出流程尚未实现，需要实机调试 UI 交互");

    // 阶段 7: 监控渲染、拉取视频、清理
    logger::step("[7/7] 渲染监控与拉取（待完善）");
    // TODO: 调用 render_monitor::wait_for_render_complete()
    //       拉取视频文件到本地
    //       清理手机端文件
