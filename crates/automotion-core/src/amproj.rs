...
pub fn analyze(path: &Path) -> Result<(AmprojType, String), String> {
    ...
    let mut archive = ZipArchive::new(file).map_err(|e| format!("无法解析 zip: {e}"))?;
    ...
    // 提取 type 属性
    let proj_type = if let Some(caps) = Regex::new(r#"type="(\w+)""#).unwrap().captures(&header) {
        // 要注意区分 <scene type="element"> 和 <media type="image/png">
        // <scene> 标签的 type 在行首区域
        if header.contains(r#"type="element""#)
            && header.find(r#"type="element""#).unwrap_or(usize::MAX)
    ...
    logger::info(&format!("amproj 分析: 类型={proj_type}, 标题=\"{title}\""));
